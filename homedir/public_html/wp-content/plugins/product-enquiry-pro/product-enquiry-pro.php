...
function pluginSettingsLinkError()
{
    if ($_GET['page'] == 'quoteup-for-woocommerce') {
        ...
        $message .= '<p>';
        $message .= sprintf(__('Please enter license key %s here %s to activate Product Enquiry Pro.', QUOTEUP_TEXT_DOMAIN), '<a href="plugins.php?page=pep-license">', '</a>');
        $message .= '</p>';
        $message .= '</div><!-- /.error -->';
        wp_die($message, 403);
    }
